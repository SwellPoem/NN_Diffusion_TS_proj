...
class DecoderBlock(nn.Module):
    def __init__(self, n_channel, n_feat, embd_dim=1024, n_head=16, attn_drop_prob=0.1, resid_drop_prob=0.1, mlp_hidden_times=4, activation='GELU', condition_dim=1024):
        ...
        self.norm1 = AdaptiveLayerNorm(embd_dim)
        ...
        self.norm3 = AdaptiveLayerNorm(embd_dim)
    ...
    def forward(self, x, encoder_output, timestep, mask=None, label_emb=None):
        a, att = self.self_attention(self.norm1(x, timestep, label_emb), mask=mask)
        x = x + a
        a, att = self.cross_attention(self.norm3(x, timestep), encoder_output, mask=mask)
        x = x + a
